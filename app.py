...
if analyze_btn:
    if not all([item, reason, claim, goal]):
        ...
        try:
            ...
            if response and response.text:
                ...
                with tab1:
                    report_main = response.text.split("[참고 규정")[0]
                    st.markdown(f"<div class='result-container'>{report_main}</div>", unsafe_allow_html=True)
                with tab2:
    ...
    st.markdown("""
        <div style='padding: 80px 40px; background-color: white; border-radius: 20px; border: 1px solid #f0f2f6; text-align: center;'>
            ...
            <h3 style='color: #0054A6; font-size: 24px; margin-bottom: 20px; font-weight: 700;'>
                품질검사 현안을 객관적 규정으로 지원합니다.
            </h3>
            <p style='color: #4a5568; font-size: 17.5px; line-height: 1.9; max-width: 850px; margin: 0 auto;'>
                협력사와의 품질 기준 해석 차이나 난항 상황에 대해<br>
                <b>한국철도공사 사규, 기술규격 및 국가계약법</b>을 근거로<br>
                실효성 있는 기술·법률 자문과 객관적인 가이드라인을 제공합니다.
            </p>
            <div style='margin-top: 40px; color: #94a3b8; font-size: 15px;'>
                사이드바에 현안 데이터를 입력한 후 버튼을 클릭하면 분석이 시작됩니다.
            </div>
        </div>
    """, unsafe_allow_html=True)
